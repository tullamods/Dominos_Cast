...
local function SetSelectorInfo(button, mType, filePath, fileName)
...
function Update_MediaButtons(self)
	...
	for i,button in pairs(self.buttons) do
		...
		if index <= #list then
			...
			SetSelectorInfo(button, mType, filePath, fileName)
		else
			button:Hide()
		end
	end
...
local function NewMediaButton(panel, name, mediaType, get, set)
	...
	b:SetScript("OnShow", function()
		...
		SetSelectorInfo(b.texture, mtype, SML:Fetch(mtype, got(panel.owner)), got(panel.owner))
	end)
	b:SetSize(75, 20)
	b:SetText(name)
